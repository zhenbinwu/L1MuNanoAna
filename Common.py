...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Calculating Efficiency ~~~~~
    ...
    def __GetDispGenMuon__(self, event):
        ...
        barrel_etaStar = akgetSt2Eta(event["GenPart_lXY"],
                                     event["GenPart_vz"],
                                     event["GenPart_eta"],
                                    )
        barrel_phiStar = akgetSt2Phi(event["GenPart_vx"],
                                     event["GenPart_vy"],
                                     event["GenPart_phi"],
                                    )
    ...
    def __bookEff__(self, name, nbins, xfirst, xlast, label=None):
        if name not in self.effnames:
            self.effnames.append((name, label))
        if name+"__den_" not in self.h:
            self.h["%s__den_" % name ] = Hist.new.Reg(nbins, xfirst, xlast,
                                                      name= "% s__den_" % name,
                                                     label=label) .Double()
        if name+"__num_" not in self.h:
            self.h["%s__num_" % name ] = Hist.new.Reg(nbins, xfirst, xlast,
                                                      name= "% s__num_" % name,
                                                     label =label) .Double()
        if name+"_eff" not in self.h:
            self.h["%s_eff" % name ] = Hist.new.Reg(nbins, xfirst, xlast,
                                                      name= "% s_eff" % name,
                                                     label =label).Weight()



    def __FillEff__(self, name, att, nbins, xfirst, xlast, label=None, gencut=None, objcut=None ):
        self.__bookEff__(name, nbins, xfirst, xlast, label )
        ...
        numerator_genidx=newgen_pass[xs.obj[(newobj_pass[xs.obj] == objcut_idx[xs.cut])]]
        numerator_objidx=newobj_pass[xs.obj[(newobj_pass[xs.obj] == objcut_idx[xs.cut])]]
    ...
    def __CalDefaultEff__(self):
        for cut in pTthresholds:
            ...
            self.__FillEff__("phi_pt_%d" % cut, "phi", 200, -4, 4, label="gen#mu #phi",
                             gencut = self.genmu.pt>(10+cut), 
                             objcut = self.pt > cut
                            )
            for region, etas in MuonEtamap.items():
                self.__FillEff__("%s_pt_%d" % (region, cut), "pt", 50, 0, 100, 
                                 label="%s gen#mu p_{T}" % region,
                                 gencut = (abs(self.genmu.eta)>= etas[0]) & (abs(self.genmu.eta) < etas[1]),
                                 objcut = self.pt > cut
                                )
                self.__FillEff__("%s_phi_%d" % (region, cut), "phi", 200, -4, 4, 
